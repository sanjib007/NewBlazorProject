...
namespace L3T.Infrastructure.Helpers.Repositories.Implementation.MenuSetupAndPermission
{
    public class MenuSetupRepository : IMenuSetupRepository
    {
		...
		private readonly ChangeRequestDataContext _CRcontext;
		public MenuSetupRepository(ApplicationMenuAndRoleWiseMenuPermissionDBContext context, ILogger<MenuSetupRepository> logger, ChangeRequestDataContext cRcontext)
		{
			_context = context;
			_logger = logger;
		...
		public async Task AutoUpdateCrApprovedToInProgress()
		{
			try
			{
				List<AutoUpdateCrApprovedToInProgressResponseModel> allCr = new List<AutoUpdateCrApprovedToInProgressResponseModel>();
				string strQuery = $@"SELECT c.Id, a.CrId, MAX(c.Subject) as Subject, MAX(c.Status) as Status, MAX(a.StartDate) as StartDate 
                                        FROM [dbo].[ChangeRequestedInfo] c
                                        inner join [dbo].[AssignToEmployeeInfo] a on a.CrId = c.Id
                                        where CAST(a.StartDate as date) <= CAST(GETDATE() as date) and c.Status = 'Approved'
                                        group by c.Id, a.CrId
                                        order by MAX(a.StartDate) desc";

				allCr = await _CRcontext.AutoUpdateCrApprovedToInProgress.FromSqlRaw(strQuery).ToListAsync();
				if (allCr.Count > 0)
				{
                    List<string> crIdList = new List<string>();
                    foreach (var cr in allCr)
                    {
                        crIdList.Add(cr.Id.ToString());
                    }
                    string updateQuery = $@"update [dbo].[ChangeRequestedInfo] set Status = 'InProgress', 
                                            StatusDisplayName = 'In Progress', LastModifiedAt = GETDATE(), 
                                            LastModifiedBy = 'System' where id in ({String.Join(", ", crIdList)})";

                    if(crIdList.Count > 0)
                        await _CRcontext.Database.ExecuteSqlRawAsync(updateQuery);
				}
				_logger.LogInformation(@$"All Update CR List Approved to In Progress status convert {DateTime.Now} : {JsonConvert.SerializeObject(allCr)}");
